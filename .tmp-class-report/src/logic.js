import { cthulhuData, JOB_FEMALE_RATIO, jobSkillProfiles, skillTranslations, weaponsList, } from "./data.ts";
// Mulberry32 PRNG
...
export async function generatePopulation(totalPopulation, baseSeed, onProgress) {
    ...
    while (remaining > 0) {
        ...
        const p1NameIdx = Math.floor(rng() *
            (p1Gender === "male"
                ? cthulhuData.firstNamesMale
                : cthulhuData.firstNamesFemale).length);
        people.push({
            id: personId,
        ...
        if (remaining > 0 && rng() > 0.15) {
            ...
            const p2Gender = isGayCouple
                ...
                : p1Gender === "male"
                    ? "female"
                    : "male";
            const p2NameIdx = Math.floor(rng() *
                (p2Gender === "male"
                    ? cthulhuData.firstNamesMale
                    : cthulhuData.firstNamesFemale).length);
            people.push({
        ...
        for (let i = 0; i < numChildren; i++) {
            ...
            const childNames = childGender === "male"
                ? cthulhuData.firstNamesMale
            ...
            if (parent2Id !== null)
                people[parent2Id].childrenIds.push(personId);
            personId++;
            remaining--;
        }
    ...
    for (const [jobName, count] of Object.entries(jobCounts)) {
        if (["Child", "Retiree", "Housewife", "Student"].includes(jobName))
            continue;
        const femaleRatio = JOB_FEMALE_RATIO[jobName] !== undefined
            ? JOB_FEMALE_RATIO[jobName]
            : 0.15;
        const numFemale = Math.round(count * femaleRatio);
        const numMale = count - numFemale;
        for (let i = 0; i < numFemale; i++)
            femaleJobSlots.push(jobName);
        for (let i = 0; i < numMale; i++)
            maleJobSlots.push(jobName);
    }
...
function getMove(str, dex, siz, age) {
    let mov = 8;
    if (str < siz && dex < siz)
        mov = 7;
...
export function generateCthulhuStats(person, rng) {
    ...
    if (profile.statBonus) {
        if (profile.statBonus.FOR)
            str = Math.min(99, str + profile.statBonus.FOR);
        if (profile.statBonus.CON)
            con = Math.min(99, con + profile.statBonus.CON);
        if (profile.statBonus.DEX)
            dex = Math.min(99, dex + profile.statBonus.DEX);
        if (profile.statBonus.INT)
            int = Math.min(99, int + profile.statBonus.INT);
        if (profile.statBonus.POU)
            pow = Math.min(99, pow + profile.statBonus.POU);
        if (profile.statBonus.EDU)
            edu = Math.min(99, edu + profile.statBonus.EDU);
        if (profile.statBonus.APP)
            app = Math.min(99, app + profile.statBonus.APP);
    }
