...
function buildDistrictNameBank() {
    const out = [];
    for (const p of DISTRICT_NAME_PREFIXES) {
        for (const c of DISTRICT_NAME_CORES)
            out.push(`${p} ${c}`);
    }
    return out;
}
...
const SCALE_CLASS_TARGETS = {
    ...
    City: [0.06, 0.17, 0.3, 0.22, 0.14, 0.07, 0.03, 0.01],
    Metropolis: [0.08, 0.19, 0.28, 0.2, 0.13, 0.07, 0.04, 0.01],
};
function normalizeStreetRoot(name) {
    return name
        .replace(/\s+(Street|Avenue|Road|Lane|Terrace|Way|Court|Place|Row|Parade|Drive|Walk|Square|Hill|Passage)$/i, "")
        .trim();
}
function buildStreetRootBank() {
    const baseRoots = [];
    const seenBase = new Set();
    for (const name of STREET_NAME_BANK) {
        const root = normalizeStreetRoot(name);
        if (!root || seenBase.has(root))
            continue;
        seenBase.add(root);
        baseRoots.push(root);
    }
...
function buildTownNameElementBank(base, tail) {
    const out = [];
    for (const b of base) {
        for (const t of tail)
            out.push(`${b}${t}`);
...
function buildTownSuffixBank(a, b) {
    const out = [];
    for (const x of a) {
        for (const y of b)
            out.push(`${x} ${y}`);
...
const TOWN_NAME_ELEMENT_A = buildTownNameElementBank(TOWN_NAME_LEAD_BASE, TOWN_NAME_LEAD_TAIL);
const TOWN_NAME_ELEMENT_B = buildTownNameElementBank(TOWN_NAME_CORE_BASE, TOWN_NAME_CORE_TAIL);
const TOWN_NAME_ELEMENT_C = buildTownSuffixBank(TOWN_NAME_SUFFIX_A, TOWN_NAME_SUFFIX_B);
function generateSettlementName(scale, rng) {
    ...
    const partsTarget = scale === "Hamlet"
        ? (rng() < 0.7 ? 1 : 2)
        : scale === "Town"
            ? (rng() < 0.5 ? 2 : 1)
            : scale === "City"
...
function settlementScale(population) {
    if (population < 2000)
        return "Hamlet";
    if (population < 20000)
        return "Town";
...
function buildDescriptorBank(nouns) {
    const out = [];
...
const BUILDING_DESCRIPTOR_BANKS = {
...
function pickDescriptor(kind, rng) {
    const bank = BUILDING_DESCRIPTOR_BANKS[kind];
    return bank[Math.floor(rng() * bank.length)];
}
function estimateCreditRating(person) {
    const job = (person.job ?? "").toLowerCase();
    let credit = 28;
    if (!job || job === "no profession")
        credit = 8;
    if (job.includes("child") || job.includes("student"))
        credit = 6;
    if (job.includes("retiree"))
        credit = 22;
    if (job.includes("doctor") ||
    ...
    if (job.includes("unemployed") || job.includes("beggar"))
        credit = Math.min(credit, 4);
...
function statusLabelFromIndex(index) {
    return STATUS_LABELS[clamp(index, 0, STATUS_LABELS.length - 1)];
}
function wealthIndexByScaleQuantile(scale, wealthQuantile) {
    const targets = SCALE_CLASS_TARGETS[scale];
    const highToLow = [...targets].reverse(); // Luxury -> Slums
    let acc = 0;
    for (let i = 0; i < highToLow.length; i++) {
        acc += highToLow[i];
        if (wealthQuantile <= acc + 1e-9)
            return 7 - i;
    }
    return 0;
}
function parcelBaseValueScore(building, model, districtByName) {
    ...
    const districtPremium = district?.kind === "Civic Hill"
        ? 18
    ...
    const kindBase = building.kind === "Commercial"
        ? 62
        : building.kind === "Civic"
            ? 59
...
function creditToClassIndex(credit, scale) {
    let idx = 0;
    if (credit >= 90)
        idx = 7;
    else if (credit >= 70)
...
function computeBuildingClassMap(model, peopleById, districtByName) {
    ...
    for (const score of sorted) {
        ...
        if (residentCredits.length > 0) {
            ...
            if (desiredIdx > classIdx && maxCredit >= 60) {
                const delta = desiredIdx - classIdx;
                const chance = clamp(0.08 + delta * 0.16 + (maxCredit - 60) * 0.005 + (avgCredit - 45) * 0.002, 0, 0.68);
...
function districtByPoint(x, y, districts) {
    let best;
...
function randomBuildingKind(rng, districtKind) {
    const roll = rng();
...
function makeRoadName(index, vertical) {
...
function rollTerrain(scale, rng) {
    const r = rng();
...
function isWaterOccupied(terrain, coastSide, coastDepth, rect, riverBand) {
    if (terrain === "none")
        return false;
    ...
    if (terrain === "coast" && coastSide) {
        if (coastSide === "west")
            return rect.left < coastDepth;
        if (coastSide === "east")
            return rect.right > coastDepth;
        if (coastSide === "north")
            return rect.top < coastDepth;
        if (coastSide === "south")
            return rect.bottom > coastDepth;
    }
    return false;
}
function addressForLot(lotX, lotY, verticalIndex, horizontalIndex, streetOffset) {
    const number = Math.floor(lotX + lotY) * 2 + 10;
    const street = makeRoadName(verticalIndex * 7 + horizontalIndex * 13 + streetOffset, true);
    return `${number} ${street}`;
}
function makeRoadPositions(count, min, max, rng) {
    const span = max - min;
...
function averageRadius(points, center) {
    if (points.length === 0)
        return 1;
    return (points.reduce((acc, p) => acc + Math.hypot(p.x - center.x, p.y - center.y), 0) /
        points.length);
}
...
function orientedRectInsideZone(zone, preferAlongRoad, rng) {
...
function quadCellPolygon(quad, u0, u1, v0, v1) {
    return [
...
function buildRoadNodeGrid(xRoads, yRoads, center, width, height, rng) {
    const grid = [];
...
function districtKindsByUrbanModel(center, width, height, rng) {
    const cornerOptions = [
    ...
    const oppositeCorner = cornerOptions.find((c) => (c.x < width * 0.5) !== (firstCorner.x < width * 0.5) && (c.y < height * 0.5) !== (firstCorner.y < height * 0.5)) ?? cornerOptions[0];
    return [
...
function irregularDistrictPolygon(seed, baseRadius, width, height, margin, rng) {
    const points = [];
...
function convexHull(points) {
    const unique = Array.from(new Map(points.map((p) => [`${p.x.toFixed(3)}:${p.y.toFixed(3)}`, p])).values());
    if (unique.length <= 3)
        return unique;
...
function generateGrowthFront(center, width, height, margin, scale, rng) {
    const points = [center];
    const branches = [];
    const initialBranches = scale === "Metropolis" ? 8 : scale === "City" ? 7 : scale === "Town" ? 6 : 5;
    const maxEnergy = scale === "Metropolis" ? 46 : scale === "City" ? 38 : scale === "Town" ? 30 : 22;
...
function minDistanceToPoints(p, cloud) {
    let best = Number.POSITIVE_INFINITY;
...
function residentialCapacity(building) {
    const b = polygonBounds(building.structurePoints);
    const area = (b.right - b.left) * (b.bottom - b.top);
    if (building.kind === "Residential")
        return clamp(Math.floor(area / 28), 2, 10);
    if (building.kind === "Mixed")
        return clamp(Math.floor(area / 36), 1, 6);
    if (building.kind === "Commercial")
        return clamp(Math.floor(area / 80), 0, 2);
    return 0;
}
function workerCapacity(building) {
    ...
    if (building.kind === "Industrial")
        return clamp(Math.floor(area / 12), 6, 42);
    if (building.kind === "Commercial")
        return clamp(Math.floor(area / 18), 3, 28);
    if (building.kind === "Civic")
        return clamp(Math.floor(area / 20), 2, 20);
    if (building.kind === "Mixed")
        return clamp(Math.floor(area / 22), 2, 16);
    return 0;
}
function hasPaidProfession(person) {
    const job = (person.job ?? "").toLowerCase();
    if (!job)
        return false;
    return !(job === "no profession" ||
...
function assignParcelOccupancy(buildings, people, scale, rng) {
    for (const b of buildings) {
        b.residents = [];
        b.workers = [];
    }
    const homeCandidates = buildings.filter((b) => b.kind === "Residential" || b.kind === "Mixed" || b.kind === "Commercial");
    const homePool = homeCandidates.length > 0 ? homeCandidates : buildings;
    const homeCap = new Map(homePool.map((b) => [b.id, residentialCapacity(b)]));
    const familyGroups = new Map();
    for (const p of people) {
        const key = p.parentIds.length > 0
    ...
    for (const p of people) {
        if (hasHome.has(p.id))
            continue;
        const fallback = homePool.find((b) => (homeCap.get(b.id) ?? 0) > 0) ??
            homePool[Math.floor(rng() * homePool.length)];
        fallback.residents.push(p.id);
        homeCap.set(fallback.id, Math.max(0, (homeCap.get(fallback.id) ?? 0) - 1));
    }
    const workCandidates = buildings.filter((b) => b.kind === "Industrial" ||
        b.kind === "Commercial" ||
        b.kind === "Civic" ||
        b.kind === "Mixed");
    const workPool = workCandidates.length > 0 ? workCandidates : buildings;
    const workCap = new Map(workPool.map((b) => [b.id, workerCapacity(b)]));
    const unemploymentRate = scale === "Hamlet"
        ? 0.1
        : scale === "Town"
            ? 0.08
            : scale === "City"
                ? 0.11
                : 0.13;
    const marriedFemaleWorkChance = scale === "Hamlet"
        ? 0.14
        : scale === "Town"
            ? 0.18
            : scale === "City"
                ? 0.24
                : 0.3;
    const pickWorkBuilding = (person) => {
        const job = (person.job ?? "").toLowerCase();
        let preferredKinds = ["Commercial", "Mixed", "Civic", "Industrial"];
...
function createMapModel(population, seed, people) {
    ...
    const xRoads = makeRoadPositions(config.verticalRoads, margin, config.width - margin, rng);
    const yRoads = makeRoadPositions(config.horizontalRoads, margin, config.height - margin, rng);
    const nodes = buildRoadNodeGrid(xRoads, yRoads, center, config.width, config.height, rng);
    ...
    if (terrain === "river") {
        water.push({
            kind: "river",
            path: `M ${margin - 18} ${(riverY - riverHalf * 0.65).toFixed(1)} C ${(config.width * 0.2).toFixed(1)} ${(riverY - riverHalf).toFixed(1)} ${(config.width * 0.55).toFixed(1)} ${(riverY + riverHalf).toFixed(1)} ${(config.width - margin + 12).toFixed(1)} ${(riverY + riverHalf * 0.45).toFixed(1)} L ${(config.width - margin + 12).toFixed(1)} ${(riverY + riverHalf * 1.1).toFixed(1)} C ${(config.width * 0.58).toFixed(1)} ${(riverY + riverHalf * 1.38).toFixed(1)} ${(config.width * 0.25).toFixed(1)} ${(riverY + riverHalf * 0.15).toFixed(1)} ${(margin - 18).toFixed(1)} ${(riverY + riverHalf * 0.8).toFixed(1)} Z`,
    ...
    if (terrain === "coast") {
        ...
        coastDepth = Math.floor((coastSide === "east" || coastSide === "west"
            ? config.width
            : config.height) *
            (0.16 + rng() * 0.12));
        if (coastSide === "east") {
    ...
    const districtSeedDefs = districtKindsByUrbanModel(center, config.width, config.height, rng);
    const districts = [];
    for (let i = 0; i < districtSeedDefs.length; i++) {
        ...
        districts.push({
            id: i,
            name: pickNameFromBank(neighborhoodNames, i, districtNameOffset, districtNameStride),
            kind: d.kind,
            hue: districtHue[d.kind],
            center: d.center,
            points: irregularDistrictPolygon(d.center, baseRadius, config.width, config.height, margin, rng),
        });
    }
    const roads = [];
    let roadId = 0;
    ...
    const buildings = [];
    let buildingId = 0;
    const growthFront = generateGrowthFront(center, config.width, config.height, margin, scale, rng);
    const shoreline = coastSide === null
        ? 0
        : coastSide === "east"
    ...
    for (const entry of blockEntries) {
        if (buildings.length >= config.maxBuildings)
            break;
        ...
        if (cellW < 22 || cellH < 22)
            continue;
        const inWater = isWaterOccupied(terrain, coastSide, shoreline, { left: b.left, right: b.right, top: b.top, bottom: b.bottom }, terrain === "river" ? { y: riverY, h: riverHalf } : undefined);
        if (inWater && rng() < 0.72)
            continue;
        const district = districtByPoint((b.left + b.right) * 0.5, (b.top + b.bottom) * 0.5, districts) ||
            districts[0];
        const localCenter = polygonCentroid(block);
        const normCenterDist = Math.hypot(localCenter.x - center.x, localCenter.y - center.y) /
        ...
        for (let u = 0; u < cols; u++) {
            for (let v = 0; v < rows; v++) {
                ...
                if (!edgeLot && rng() > (scale === "Metropolis" ? 0.45 : scale === "City" ? 0.32 : 0.18))
                    continue;
                const lot = quadCellPolygon(block, u / cols, (u + 1) / cols, v / rows, (v + 1) / rows);
                const lb = polygonBounds(lot);
                if (lb.right - lb.left < 7 || lb.bottom - lb.top < 7)
                    continue;
                ...
                buildings.push({
                    ...
                    address: addressForLot(xi * 20 + rng() * 4, yi * 12 + rng() * 4, xi, yi, addressStreetOffset),
                    residents: [],
                    workers: [],
                });
            }
    ...
    for (const district of districts)
        districtPoints.set(district.id, []);
    for (let yi = 0; yi < nodes[0].length - 1; yi++) {
        for (let xi = 0; xi < nodes.length - 1; xi++) {
            const block = [
                nodes[xi][yi],
    ...
    for (const district of districts) {
        ...
        if (!pts || pts.length < 3)
            continue;
        district.points = insetPolygon(convexHull(pts), -(2 + rng() * 8)).map((p) => ({
...
export function debugClassDistribution(population, seed, people) {
    const model = createMapModel(population, seed, people);
    const peopleById = new Map(people.map((p) => [p.id, p]));
    const districtByName = new Map(model.districts.map((d) => [d.name, d]));
    const classByBuilding = computeBuildingClassMap(model, peopleById, districtByName);
    const counts = Object.fromEntries(STATUS_LABELS.map((label) => [label, 0]));
    for (const label of classByBuilding.values())
...
function ensureCamera(svgEl, width, height) {
...
function updateRoadLabelVisibility(svgEl) {
    const cam = cameraBySvg.get(svgEl);
    if (!cam)
        return;
    const labels = svgEl.querySelectorAll(".town-map-road-label");
    labels.forEach((el) => {
        const tier = (el.dataset.tier || "local");
        const minZoom = tier === "arterial" ? 1 : tier === "collector" ? 1.6 : 2.4;
        el.style.display = cam.zoom >= minZoom ? "" : "none";
    });
    const neighborhoods = svgEl.querySelectorAll(".town-map-neighborhood-label");
...
function setZoom(svgEl, nextZoom) {
    const cam = cameraBySvg.get(svgEl);
    if (!cam)
        return;
...
export function initTownMapControls(svgEl, zoomInBtn, zoomOutBtn, zoomResetBtn) {
    if (svgEl.dataset.mapControlsReady === "1")
        return;
...
function districtFillClass(kind) {
    switch (kind) {
        case "Old Quarter":
            return "district-old";
        case "Market Ward":
...
function buildingFillClass(kind) {
    switch (kind) {
        case "Residential":
...
function roadTierClass(tier) {
    if (tier === "arterial")
        return "town-map-road-arterial";
    if (tier === "collector")
        return "town-map-road-collector";
    return "town-map-road-local";
}
function hoverPeopleButtons(title, ids, peopleById, getPersonName) {
    if (ids.length === 0)
        return "";
...
function roadLabel(svgEl, road) {
    const points = Array.from(road.path.matchAll(/(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)/g)).map((m) => ({ x: Number(m[1]), y: Number(m[2]) }));
    if (points.length < 2)
        return;
    ...
    text.setAttribute("transform", `rotate(${angle.toFixed(1)} ${mid.x.toFixed(1)} ${mid.y.toFixed(1)})`);
    text.textContent = road.name;
    svgEl.appendChild(text);
}
export function renderTownMapPrototype(params) {
    const { svgEl, titleEl, legendEl, statsEl, hoverEl, scaleEl, population, seed, people, getPersonName, onPersonClick, onPersonAddressIndex, } = params;
    const model = createMapModel(population, seed, people);
  ...
  `;
    ...
    for (const district of model.districts) {
        ...
        const neighborhood = document.createElementNS("http://www.w3.org/2000/svg", "text");
    ...
    for (const water of model.water) {
        ...
        const labelX = water.labelX ?? (water.kind === "river" ? model.width * 0.47 : model.width * 0.82);
        const labelY = water.labelY ?? (water.kind === "river" ? model.height * 0.39 : model.height * 0.13);
    ...
    const upsertAddress = (personId, type, address) => {
    ...
    const districtByName = new Map(model.districts.map((d) => [d.name, d]));
    const socialClassByBuildingId = computeBuildingClassMap(model, peopleById, districtByName);
    legendEl.innerHTML = model.districts
        .map((d) => `<div class="town-map-legend-row">
        <span class="town-map-legend-chip" style="background:${d.hue}"></span>
        <span>${d.name}</span>
      </div>`)
        .join("");
    const residential = model.buildings.filter((b) => b.kind === "Residential" || b.kind === "Mixed").length;
    const employment = model.buildings.filter((b) => b.kind === "Commercial" || b.kind === "Industrial" || b.kind === "Civic").length;
  ...
  `;
    const defaultHoverHtml = '<div class="town-map-hover-empty">Hover a parcel to inspect residents and workers. Click a parcel to pin details.</div>';
    hoverEl.innerHTML = defaultHoverHtml;
    const renderBuildingPanel = (building) => {
        const socialClass = socialClassByBuildingId.get(building.id) ?? "Working Class";
    ...
    `;
        hoverEl
            .querySelectorAll(".town-map-person-link")
            .forEach((btn) => {
    ...
    const setSelectedBuilding = (buildingId) => {
        ...
        const nodes = svgEl.querySelectorAll(".town-map-building");
        nodes.forEach((n) => {
    ...
    const interactiveBuildings = svgEl.querySelectorAll(".town-map-building");
    ...
    svgEl.onclick = (event) => {
        ...
        if (target.closest(".town-map-building"))
            return;
        if (selectedBuildingId !== null) {
            setSelectedBuilding(null);
        }
